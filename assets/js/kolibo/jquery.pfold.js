	...
	*/
	...
	$.PFold.prototype = {
		...
		_init : function( options ) {
			...
			// background layer
			this.$modalBackground = $('#background-layer');
		...
		_initEvents : function() {
			...
			/*
			this.$modalBackground.on('click', function(){
				self.fold();
			});
			*/
		...
		_getTranslationViewport : function() {
			...
			var fixedOffset = ($(window).height() - this.$finalEl.outerHeight()) / 2;
		...
		// method to lock scrolling in window
		_scrolling : function($style){
			//$('body').css('overflow', $style);
		},
		// add closing button to unfolded 
		_addCloseButton : function(){
			var self = this;

			this.$finalEl.append('<span class="entypo-cancel"></span>');
			this.$close = this.$finalEl.find('.entypo-cancel');
		...
		scrollToElement	: function(){
			...
			var viewportWidth = $window.width(),
				...
				elWidth = this.$finalEl.width(),
				elHeight = this.$finalEl.height(),
				elOffset = this.$finalEl.offset();
		...
		showModalBackground	: function(show){
			// get body height to fix bg grad height
			var body_height = $('body').height();

			if (show) {
				this.$modalBackground.css({opacity: 1, visibility: "visible", height: body_height});
			}else{
				this.$modalBackground.css({opacity: 0, visibility: "hidden", height: body_height});
			}
		}
	};
